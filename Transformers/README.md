# Fusion_Midas_Flava — Transformers (Swin)

Fusion RGB-D. Profondeur GT ou MiDaS gelé (pré-calculé). Features image : **Swin Transformer**
(timm, `SWIN_MODEL` dans `config.py`). Données partagées : `../data/nutrition5k`.

> Variante Transformers du projet `Fusion_Midas_Flava` : **même architecture/code que `../CNN`**,
> seul le backbone change (Swin au lieu de ResNet-101). 4 échelles `[/4,/8,/16,/32]` au lieu de 5
> (pas de niveau stem). Env et `data/` partagés à la racine.
